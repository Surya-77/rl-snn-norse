...
flags.DEFINE_integer("memory_use_start", 500, "Samples to fill memory before learning")
...
class SNNQNet(torch.nn.Module):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # self.lif_2 = LIFRecurrentCell(128, 128, p=LIFParameters(method="super", alpha=100.0))
        # self.fc2 = nn.Linear(128, 128)
    ...
    def forward(self, x):
        ...
        for ts in range(seq_length):
            ...
            # z2, s1 = self.lif_2(z1[:, :], s1)
            # z2 = self.dropout(z1)
            z2 = z1
            vo, s2 = self.readout(z2, s2)
            voltages[ts, :, :] = vo

        m, _ = torch.max(voltages, 0)
        # m = torch.sum(voltages, 0)
...
def main(args):
    ...
    for e in range(FLAGS.episodes):
        ...
        if memory.size() > FLAGS.memory_use_start:
            ...
            train(policy, policy_target, memory, optimizer, device)
