...
def train_net(network):
    ...
    for i in range(K_epoch):
        td_target = r + gamma * network.v(s_prime) * done_mask
        delta = td_target - network.v(s)
        ...
        pi = network.pi(s, softmax_dim=1)
        pi_a = pi.gather(1, a)
        ratio = torch.exp(torch.log(pi_a) - torch.log(prob_a))  # a/b == exp(log(a)-log(b))

        surr1 = ratio * advantage
        surr2 = torch.clamp(ratio, 1 - eps_clip, 1 + eps_clip) * advantage
        loss = -torch.min(surr1, surr2) + F.smooth_l1_loss(network.v(s), td_target.detach())
    ...
    for n_epi in range(10000):
        ...
        while not done:
            for t in range(T_horizon):
                ...
                put_data(model, (s, a, r / 100.0, s_prime, prob[a].item(), done))
                s = s_prime
